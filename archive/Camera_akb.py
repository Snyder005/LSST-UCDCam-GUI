...
class Camera(object):
    ...
    def MultiExpose(self):
        ...
        if num_per_increment < 1 or num_per_increment > 100:
            print "Number of exposures per increment must be an integer between 1 and 100.  Exposures not done."
            return
	...
	###!!!! Assumes stages have been zeroed at location where dithering is desired!!!! 
            ...
            for exposure_counter in range(numinc):
                for sub_counter in range(num_per_increment):     
                    ...
                    if dither_radius > 0 and num_per_increment > 1:
                        x_dither = int(dither_radius * (-1.0 + 2.0 * numpy.random.rand()))
                        y_dither = int(dither_radius * (-1.0 + 2.0 * numpy.random.rand()))
			z_dither = int(dither_radius * (-1.0 + 2.0 * numpy.random.rand()))
			foo_dithers=[x_dither,y_dither,z_dither]
			#self.stage.Read_Encoders()
			for i in range(3):
			    thepos=self.stage.read_pos[i]/2.5

			    if abs(thepos-foo_dithers[i])>dither_radius:
				foo_dithers[i]=int(-1.*thepos[i])
				"""if i==3:
				    zpos_mic=self.stage.read_pos[i]
				    foo_dithers[i]=int(-1.*zpos_mic/2.5 + numpy.sign(zpos_mic)*30) # purposeful movement of z-stage back by 5x offset from zero
				else: foo_dithers[i]=int(-1.*self.stage.read_pos[i]/2.5)"""
    ...
    def Define_Frame(self):
        ...
        self.sensor_id_ent.insert(0,'114-04')
        ...
        increment_type = OptionMenu(self.frame, self.increment_type, "None", "X", "Y", "Z", "Exp(Log)", "Exp(Linear)","V-curve (Linear)")
        ...
        stop_exposures_but = Button(self.frame, text="Stop Exposures", width=16,command=lambda:self.StopExposures())
        stop_exposures_but.grid(row=10,column=3)
        multi_capture_but = Button(self.frame, text="Start Exposures\nNumber of Exposures = \n # of Increments * # Per Increment", width=26,command=lambda:self.MultiExpose())
