...
class Camera(object):
    ...
    def Define_Frame(self):
        ...
        self.sensor_id_ent.insert(0,'ITL-3800C-002')
    ...
    def exp_acq(self, exptime=0.0, fitsfilename='test.fits'):
        ...
        if not self.CheckIfFileExists(FlushFile):
            ...
            return
        TimingFile = eolib.getCfgVal(self.CfgFile,"TIM_FILE")
        if not self.CheckIfFileExists(TimingFile):
            print "Timing File not found.  Exiting exp_acq"
            return
        ...
        self.runcmd([self.edtsaodir+"/edtwriteblk", "-f", FlushFile])     # load the file to flush the array
        time.sleep(5.0)   # delay for array to flush

        #print "After edtwriten, time = ",time.time()
        self.runcmd([self.edtsaodir+"/edtwriteblk", "-f", NoFlushFile])     # load the signal file to stop flushing
        ...
        self.runcmd([self.edtsaodir+"/edtwriteblk", "-f", FlushFile])     # load the signal file to restart flushing
