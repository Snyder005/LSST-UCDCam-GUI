...
def ReadParticleLog(filename):
    ...
    lines = GetLastNLines(filename, 4500)
    Time = []
    Small = []
    Big = []
...
def MakePlot(Time, Small, Big, ExtTime, Ext, TempTime, Time_A, Time_B):
    ...
    MinTime = Time[-1]-29.0
    MaxTime = MinTime + 30.0
    MinY = 0.0
    MaxY = 7.0
    figure(figsize=(8,10))
    ax1=axes([0.2,0.2,0.7,0.7])
    ax1.set_title("Camp Fire Impact on UC Davis Particle Count Trend", fontsize = 16)
    ax1.set_xlabel("Time (Julian Day)")
    #ylabel("Log Particles/m^3")
    ax1.text(MinTime-3.5,4.5,"Log Particles/m^3", color='black', rotation = 'vertical')
    ax1.text(MinTime-2.0,3.3,">2.5 $\mu m$", color='green', rotation = 'vertical')
    ax1.text(MinTime-2.0,4.7,">0.5 $\mu m$", color='blue', rotation = 'vertical')
    ...
    fire_start = Date_to_JD(datetime.datetime(2018,11,8,4,0,0))
    ax1.plot([fire_start,fire_start],[MinY,MaxY],linestyle = 'dotted', color = 'red', lw=4)
    ax1.text(fire_start+1,6.5, 'Fire Start', color='red', fontweight = 'bold')

    rain_start = Date_to_JD(datetime.datetime(2018,11,22,4,0,0))
    ax1.plot([rain_start,rain_start],[MinY,MaxY],linestyle = 'dotted', color = 'green', lw=4)
    ax1.text(rain_start-5,6.5, 'Rain Start', color='green', fontweight = 'bold')
            ...
            if lake.Temp_A > Temp_to_Fill:
                ...
                if jd - float(fill_lines[0].split()[0]) > 0.125:
                    ...
                    if start_fill_status:
                        ...
                        time.sleep(0.1)
                        while elapsed < Fill_Time_Limit and difftemp < 0.8 and temp > Overflow_Temp_Limit:
