...
def GetExteriorCounts():
    ...
    try:
        response = urllib2.urlopen('http://www.arb.ca.gov/aqmis2/display.php?download=y&param=PM25HR&units=001&year=2017&report=SITE1YR&statistic=DAVG&site=2143&ptype=aqd&monitor=-', timeout=10)
        data = response.read()
        response.close()
    except Exception as e:
...
def MakePlot(Time, Small, Big, ExtTime, Ext, TempTime, Time_A, Time_B):
    ...
    for line in fill_lines:
        ...
        if plottime > MinTime:
            ...
            ax2.text(plottime-0.30, -192.6, '%.0f'%fill_time, color='brown')
    ax3=ax2.twinx()
    ax3.set_ylim(-140,40)
...
for i in [-1,-2,-3]:
    print TempTime[i], Temp_A[i], Temp_B[i]
#************************************* END MAIN PROGRAM ***********************************************
