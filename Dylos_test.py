...
def GetExteriorCounts():
    ...
    try:
        response = urllib2.urlopen('http://www.arb.ca.gov/aqmis2/display.php?download=y&param=PM25HR&units=001&year=2016&report=SITE1YR&statistic=DAVG&site=2143&ptype=aqd&monitor=-')
        data = response.read()
        response.close()
    except Exception as e:
...
def MakePlot(Time, Small, Big, ExtTime, Ext, TempTime, Time_A, Time_B):
    ...
    try:
        command = 'scp particle_graph.png cslage@phys-web.physics.ucdavis.edu:/sites/lage.physics.ucdavis.edu/support_files/astro_papers/'
